...
var questionsRouter = createTRPCRouter({
  list: protectedProcedure.input(z.object({
    ...
    if (input.auditada !== void 0) filters.push(eq(questions.auditada, input.auditada));
...
Responda em JSON puro (sem markdown, sem bloco de c\xF3digo) com exatamente esta estrutura:
{
  ...
  calibrateTri: adminProcedure.input(z.object({
    // Se passado, calibra só essas questões; senão calibra TODAS as ativas
    ids: z.array(z.number().int().positive()).optional()
  })).mutation(async ({ ctx, input }) => {
    const TRI_MAP = {
      "Muito Baixa": { a: 0.6, b: -2, c: 0.2 },
      "Baixa": { a: 0.8, b: -1.5, c: 0.2 },
      "M\xE9dia": { a: 1, b: -0.5, c: 0.2 },
      "Alta": { a: 1.2, b: 0.5, c: 0.2 },
      "Muito Alta": { a: 1.5, b: 1.5, c: 0.2 }
    };
    const where = input.ids?.length ? and(eq(questions.active, true), inArray(questions.id, input.ids)) : eq(questions.active, true);
    const rows = await ctx.db.select({ id: questions.id, nivel: questions.nivel_dificuldade }).from(questions).where(where);
    let updated = 0;
    for (const row of rows) {
      const params = TRI_MAP[row.nivel ?? "M\xE9dia"] ?? TRI_MAP["M\xE9dia"];
      await ctx.db.update(questions).set({ param_a: params.a, param_b: params.b, param_c: params.c }).where(eq(questions.id, row.id));
      updated++;
    }
    return { updated };
  }),
  ...
  applyAuditFixes: adminProcedure.input(z.object({
    ...
    if (fields.conteudo_principal && CONTEUDOS_VALIDOS.includes(fields.conteudo_principal)) {
      await ctx.db.update(questions).set({ conteudo_principal: fields.conteudo_principal }).where(eq(questions.id, id));
...
async function drawQuestions(db2, count, excludeIds = [], fonte) {
  ...
  const allIds = await db2.select({ id: questions.id }).from(questions).where(whereClause);
  ...
  const rows = await db2.select({
    ...
    tags: questions.tags
  }).from(questions).where(inArray2(questions.id, selected));
  ...
  start: protectedProcedure.input(
    ...
    if (drawn.length < MIN_QUESTIONS) {
      throw new TRPCError3({
        ...
        message: `Banco insuficiente para este simulado. Dispon\xEDvel: ${drawn.length} quest\xE3o(\xF5es). M\xEDnimo necess\xE1rio: ${MIN_QUESTIONS}.`
      });
    }
...
async function collectStudentData(ctx) {
  ...
  if (challenges.length > 0) {
    const allQIds = [...new Set(challenges.flatMap((c) => c.questionIds))];
    if (allQIds.length > 0) {
      const qDetails = await ctx.db.select({ id: questions.id, conteudo: questions.conteudo_principal, tags: questions.tags, gabarito: questions.gabarito }).from(questions).where(inArray5(questions.id, allQIds));
      const qMap = new Map(qDetails.map((q) => [q.id, q]));
      for (const ch of challenges) {
        const answers = ch.answers;
        for (const [qIdStr, selected] of Object.entries(answers)) {
