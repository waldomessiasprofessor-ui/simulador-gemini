...
import { Plus, Pencil, Trash2, Eye, EyeOff, ChevronDown, ChevronUp, Loader2, Search, X, Save, Tag, FileCode2, ClipboardPaste, CheckCircle2, Sparkles, AlertTriangle, ThumbsUp, ThumbsDown, Info } from "@/icons";
...
export default function AdminQuestoes() {
  ...
  function startEdit(q: any) {
    ...
    setShowForm(true);
    setTimeout(() => {
      document.getElementById("admin-question-form")?.scrollIntoView({ behavior: "smooth", block: "start" });
    }, 50);
  }
  ...
  return (
    <div className="space-y-6 py-2">
      ...
      {/* Formulário */}
      {showForm && (
        <div id="admin-question-form" className="rounded-2xl p-6 space-y-5" style={{ background: "#fff", border: `1.5px solid ${editId ? "#01738d" : "#E2D9EE"}` }}>
          <div className="flex items-center justify-between">
            <div>
              ...
              {editId && (
                <p className="text-xs mt-0.5" style={{ color: "#01738d" }}>
                  Editando questão existente — alterações sobrescrevem os dados atuais
                </p>
              )}
            </div>
